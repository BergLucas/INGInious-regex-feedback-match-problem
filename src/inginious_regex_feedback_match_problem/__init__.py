...
class RegexFeedbackStatic(INGIniousPage):
    """Serve static files for the regex feedback plugin."""
...
class RegexFeedbackMatchProblem(Problem):
    """Display an input box and check that the content match one of regexes."""
    ...
    def __init__(
        ...
        """Initialises a RegexFeedbackMatchProblem.
        ...
        self._regexes: list[dict] = list(content.get("regexes", ()))
    ...
    def parse_problem(cls, problem_content: dict[str, Any]) -> dict[str, Any]:
        ...
        if "regexes" in problem_content:
            problem_content["regexes"] = [
                val
                for _, val in sorted(
                    iter(problem_content["regexes"].items()), key=lambda x: int(x[0])
                )
            ]
            for data in problem_content["regexes"]:
                if "valid" in data:
                    data["valid"] = True
...
class DisplayableMatchProblem(RegexFeedbackMatchProblem, DisplayableProblem):  # type: ignore
    """A displayable regex feedback match problem."""
    ...
    def show_input(  # type: ignore  # noqa: D415
        ...
        """Show a RegexFeedbackMatchProblem.
        ...
        return template_helper.render(
            "tasks/regex_feedback_match.html",
    ...
    def show_editbox(  # type: ignore
        ...
        """Show the edit box for a RegexFeedbackMatchProblem.
        ...
        """
        return template_helper.render(
            "course_admin/subproblems/regex_feedback_match.html",
            template_folder=PATH_TO_TEMPLATES,
            key=key,
        )
    ...
    def show_editbox_templates(  # type: ignore
        ...
        """Show the edit box templates for a RegexFeedbackMatchProblem.
        ...
        """
        return template_helper.render(
            "course_admin/subproblems/regex_feedback_match_templates.html",
            template_folder=PATH_TO_TEMPLATES,
            key=key,
        )
...
def init(
    ...
    """Initialises the RegexFeedbackMatchProblem plugin.
    ...
    plugin_manager.add_page(
        "/plugins/regex_feedback_match/static/<path:path>",
        RegexFeedbackStatic.as_view("regex_feedback_match_static"),
    )
    plugin_manager.add_hook(
        "javascript_header",
        lambda: "/plugins/regex_feedback_match/static/js/studio.js",
    )
    plugin_manager.add_hook(
        "javascript_header",
        lambda: "/plugins/regex_feedback_match/static/js/task.js",
    )
    task_factory: TaskFactory = course_factory.get_task_factory()
    task_factory.add_problem_type(DisplayableMatchProblem)
